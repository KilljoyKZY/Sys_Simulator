simul_gene_bikes_station_based <- function(city, env='local'){
      ...
      for (m in 0:59){
        ...
        if (nrow(end_sub)>0){
          for (i in 1:nrow(end_sub)){
            ...
            # if ((scen_df_reb$avai_bike[grid_id] >= grid_cap_rate * scen_df_reb$Bikes_fina[grid_id]) &
            #     (scen_df_reb$reb_status[grid_id] == 'none') &
            #     (scen_df_reb$avai_bike[grid_id] >= grid_cap_rate * initial_bike_cap)) {
            if ((scen_df_reb$avai_dock[grid_id] == 0) &
                (scen_df_reb$reb_status[grid_id] == 'none') &
